package com.danram.server.aspect;
...
public class LoggerAspect {
    ...
    @Before("execution(* com.danram.server..*(..))")
    public void before(JoinPoint joinPoint) {
        log.info("[START] | where: " + joinPoint.toString());
    }

    @AfterReturning("execution(* com.danram.server..*(..))")
    public void afterReturning(JoinPoint joinPoint) {
        log.info("[END] | where: " + joinPoint.toString());
    }

    @AfterThrowing(value = "execution(* com.danram.server..*(..))", throwing = "exception")
